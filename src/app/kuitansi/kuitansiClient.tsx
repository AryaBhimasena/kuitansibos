...
export default function KuitansiOrchestratorPage() {
  ...
  const title = searchParams.get("title") ?? "Kuitansi";
  const jenis = mapJenisKuitansi(searchParams.get("jenis"));
...
useEffect(() => {
  ...
  fetch(
    `https://script.google.com/macros/s/AKfycbxRd37N6y8vsTgdtW0xcl9f3HWqUu4r1pwWr_LQeqb4bBUOm9k9XzXQqcGvdq2WzPm64w/exec?action=getKuitansiById&id=${id}`
  )
    .then(res => res.json())
    .then(json => {
      console.log("EDIT DATA:", json); // debug dulu
      if (json.success) {
        setPayload(json.data);
      }
    })
    .catch(err => console.error("Fetch edit data error:", err));
}, [id]);
    ...
    <div className="kuitansi-orchestrator">
      <aside className="orchestrator-form">
        ...
        <div className="form-body">
          {renderKuitansiForm(jenis, step, payload, setPayload)}
        </div>
          ...
          <div className="viewport-center">
            ...
            >
              <div className="preview-paper">
                <div className="paper-content">{renderKuitansiPreview(jenis, payload, previewPage)}</div>
              </div>
				<div
				  id="pdf-export-root"
				  style={{
				...
				>
				  {renderAllKuitansiPreview(jenis, payload)}
				</div>
